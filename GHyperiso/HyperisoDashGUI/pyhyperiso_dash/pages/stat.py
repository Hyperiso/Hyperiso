...
def _described_enum_options(enum_cls, descriptions: dict[str, str]) -> list[dict[str, str]]:
    return [
...
PROFILING_OPTIONS = _described_enum_options(ProfilingMethod, {
...
CONTOUR_ALGORITHM_OPTIONS = _described_enum_options(ContourAlgorithm, {
...
PROFILER_MODE_OPTIONS = _described_enum_options(ProfilerMode, {
    "MINUIT": "Minuit",
...
def expert_controls():
    return html.Div([
        field("Expert controls", dcc.Checklist(id="stat-advanced-mode", options=[{"label": "show decay and advanced χ² controls", "value": "advanced"}], value=[], className="checklist")),
...
def contour_controls():
    return html.Div([
        field(
            ...
            dcc.Checklist(
                id="stat-do-contour",
                options=[{"label": "compute confidence contours after the χ² fit", "value": "contour"}],
                value=["contour"],
                className="checklist",
            ),
        ),
        html.Div(id="stat-contour-controls", children=[
            small_note(
            ...
            html.Div(className="form-grid-2", children=[
                field("X-axis parameter", dropdown("stat-contour-x", [], value=None, placeholder="Add at least two fit parameters")),
                field("Y-axis parameter", dropdown("stat-contour-y", [], value=None, placeholder="Add at least two fit parameters")),
            ]),
            html.Div(className="form-grid-3", children=[
                field("Confidence levels", dropdown("stat-contour-levels", CONTOUR_LEVEL_OPTIONS, value=[1.0, 2.0], multi=True, clearable=False)),
                field("Resolution", num_input("stat-contour-resolution", 60)),
                field("Profiler mode", dropdown("stat-profiler-mode", PROFILER_MODE_OPTIONS, value="LAPLACE_NUISANCE", clearable=False)),
            ]),
            html.Div(className="form-grid-3", children=[
                field("ProfilingMethod", dropdown("stat-profiling-method", PROFILING_OPTIONS, value="SLICE", clearable=False)),
                field("ContourAlgorithm", dropdown("stat-contour-algorithm", CONTOUR_ALGORITHM_OPTIONS, value="MINUIT", clearable=False)),
                field("Fallback algorithm", dropdown("stat-contour-fallback", FALLBACK_ALGORITHM_OPTIONS, value="AMS", clearable=False)),
            ]),
            small_note("ProfilingMethod is only relevant when more than two fit parameters are present; with exactly two parameters the GUI forces SLICE.", tone="warn"),
...
def layout():
    return html.Div(children=[
        page_title("Statistics", "χ² Monte-Carlo covariance workflow: configure observables, propagate uncertainty, fit parameters, and extract 2D confidence contours.", "Stat / χ²"),
        html.Div(className="page-grid", children=[
            html.Div(className="grid", children=[
                ...
                card("Experiments and Monte Carlo", "CHI2_MC_COVARIANCE only", html.Div([
                    small_note("The GUI is locked to the χ² MC-covariance likelihood. The profiled-nuisance likelihood mode is not exposed here.", tone="info"),
                    ...
                    html.Div(id="stat-advanced-options-wrap", style={"display": "none"}, children=[
                        ...
                        field("MC nuisance pruning", dcc.Checklist(id="stat-nuisance-pruning", options=[{"label": "sensitivity pruning before MC covariance propagation", "value": "prune"}], value=["prune"], className="checklist")),
                    ]),
                ])),
            ...
            html.Div(className="grid", children=[
                ...
                card("Best-fit parameters", "χ² maximum-likelihood estimate", data_table("stat-fit-table", FIT_COLUMNS, page_size=12)),
                card("Fit correlations", "p_correlations", graph("stat-corr-fig", height=460), className="card graph-card"),
                card("2D confidence contours", "core ContourEngine paths", graph("stat-contour-fig", height=520), className="card graph-card"),
