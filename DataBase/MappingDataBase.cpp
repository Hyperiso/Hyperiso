...
void MappingDatabase::loadFromJson(const std::string& jsonFilePath) {
    ...
    // Lire tout le fichier dans une chaîne unique
    std::string content((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());

    size_t pos = 0;
    while ((pos = content.find("\"", pos)) != std::string::npos) {
        // Extraction de la clé (paramètre, ex: "m_c")
        size_t start = pos + 1;
        size_t end = content.find("\"", start);
        std::string key = content.substr(start, end - start);
        pos = end + 1;

        // Recherche du champ "block" pour ce paramètre et extraction de sa valeur
        ...
        // Recherche du champ "pdgCode" pour ce paramètre et extraction de sa valeur
        ...
        // Nettoyage et conversion de pdgCode en entier
        codeStr.erase(0, codeStr.find_first_not_of(" \t")); // Supprimer espaces initiaux
        codeStr.erase(codeStr.find_last_not_of(" ,\t") + 1); // Supprimer espaces/virgule finaux
        int pdgCode = 0;
        try {
            pdgCode = std::stoi(codeStr); // Conversion en entier
        } catch (const std::invalid_argument& e) {
            std::cerr << "Invalid integer format for pdgCode: " << codeStr << std::endl;
            pdgCode = -1; // Valeur par défaut en cas d'erreur
        } catch (const std::out_of_range& e) {
            std::cerr << "Integer out of range for pdgCode: " << codeStr << std::endl;
            pdgCode = -1; // Valeur par défaut en cas d'erreur
        }

        // Ajout du paramètre extrait à la map
        if (!key.empty() && !block.empty()) {
            paramsMap[key] = {block, pdgCode};
        }

        // Avancer après la dernière accolade pour éviter un bloc infini
