...
void SUSY_LO_Strategy::init(double scale, WilsonSet& C_match) {
	...
	LOG_INFO("epsilon b : " + std::to_string((*sus_param).epsilonb));
	LOG_INFO("epsilon bp : " + std::to_string((*sus_param).epsilonbp));
...
+((*susy)("AMIX", 00)+(*susy)("AMIX", 01)*(*susy)("HMIX",2))*(-(*susy)("AMIX", 01)+(*sus_param).epsilonb*(*susy)("AMIX", 00))*pow((*sus_param).mass_b_muW/(*susy)("MASS",36),2.)
	...
	LOG_INFO("C7H_0", C_LO[static_cast<size_t>(WilsonCoefficient::C7)] );
	...
	LOG_INFO("C7SMeps_0", C7SMeps_0);
	LOG_INFO("C7Heps_0", C7Heps_0);
	LOG_INFO("C7Heps2_0", C7Heps2_0);
	LOG_INFO("C7charg_0", C7charg_0);
	LOG_INFO("C7_chargeps_0", C7_chargeps_0);
...
void SUSY_NNLO_Strategy::init(double scale, WilsonSet& C_match) {
	...
	LOG_INFO("kapaaaaaaaaaaaaa", (*sus_param).kappa);
	...
	LOG_INFO("C4charg_2", C4charg_2);
	LOG_INFO("C5charg_2", C5charg_2);
	LOG_INFO("C5H_2",C_NNLO[static_cast<size_t>(WilsonCoefficient::C5)] );
...
void SUSY_NLO_Strategy::init_scalar(double Q_match,double Q,int gen, WilsonSet& C) {
	...
	C_NLO[static_cast<size_t>(WilsonCoefficient::CQ1)]=CQ1H_1+CQ1charg_1;
	
	C_NLO[static_cast<size_t>(WilsonCoefficient::CQ1)]/=(*sus_param).epsfac;
	...
	C_NLO[static_cast<size_t>(WilsonCoefficient::CQ2)]=CQ2H_1+CQ2charg_1;
	...
	C_NLO[static_cast<size_t>(WilsonCoefficient::CQ2)]+=CQ2four_1;
	...
	if((*susy)("MASS",46)!=0.||(*susy)("MASS",45)!=0.)
	{
		...
		C_LO[static_cast<size_t>(WilsonCoefficient::CQ1)]=0.;
		C_LO[static_cast<size_t>(WilsonCoefficient::CQ2)]=0.;
		C_NLO[static_cast<size_t>(WilsonCoefficient::CQ1)]=0.;
		C_NLO[static_cast<size_t>(WilsonCoefficient::CQ2)]=0.;
		...
		C_LO[static_cast<size_t>(WilsonCoefficient::CQ1)]=(CQ1H+CQ1c)*(*sus_param).mass_b_muW/(*sus_param).sw2/(*sus_param).epsfac;
		C_LO[static_cast<size_t>(WilsonCoefficient::CQ2)]=(CQ2H+CQ2c)*(*sus_param).mass_b_muW/(*sus_param).sw2/(*sus_param).epsfac;
