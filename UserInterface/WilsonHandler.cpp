    ...
    try {
        ...
        if (coefficient_name.empty()) {
            for (const auto& elem : *group) {
                std::complex<double> coeff = elem.second->get_CoefficientMatchingValue(order);
                std::cout << "Coefficient " << elem.first << " at Q_match = " << Q_match << " and Q = " << Q
                          << ": " << coeff.real() << " + " << coeff.imag() << "i\n";
            }
        } else {
            auto it = group->find(coefficient_name);
            if (it != group->end()) {
                std::complex<double> coeff = it->second->get_CoefficientMatchingValue(order);
                std::cout << "Coefficient " << coefficient_name << " at Q_match = " << Q_match << " and Q = " << Q
                          << ": " << coeff.real() << " + " << coeff.imag() << "i\n";
            } else {
