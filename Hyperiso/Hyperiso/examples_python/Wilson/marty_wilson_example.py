...
if __name__ == "__main__":
    ...
    config = WilsonBuildConfig(
        groups={WGroup.B},  # Groups to be build.
    ...
    interface.build(
        config
    )  # API to build the wilson coefficients specifed within the configuration.

    config.groups = [WGroup.BPrime]
    interface.add_wilson_group(
        config
    )  # You can add a new group after the build. The same QCDOrder and scales will be used.
    ...
    for coef in coefs:
        ...
        print(
            req.coefficient,
            " : ",
            interface.get_sep_order_matching(req.group, req.coefficient, req.contribution),
        )  # Get a dictionnary with all requested order and their contributions.
    ...
    for coef in coefs_primes:
        req.coefficient = coef
        print(
            req.coefficient, " : ", interface.get_FR(req)
        )  # Get full running, with sum of all order up to the requested order.
