...
if __name__ == "__main__":
    ...
    print(
        "Is VCKM a dependentblock : ",
        DependantBlockInfoProvider().is_dependent_block(ParameterType.SM, "VCKM"),
    )

    # get_source_blocks will return all blocks used in the calculation of the parameter.
    print(
        "Sources of VCKM : ",
        DependantBlockInfoProvider().get_source_blocks(ParameterType.SM, "VCKM"),
    )
    ...
    print(
        "Changed VCKM[1,1] value after reatach: ",
        pp.get_by_pid(ParamId(ParameterType.SM, "VCKM", [1, 1])),
    )
