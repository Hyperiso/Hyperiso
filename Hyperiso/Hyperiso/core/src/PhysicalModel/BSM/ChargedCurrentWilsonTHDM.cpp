...
C_S1_bc_THDM::C_S1_bc_THDM()
    : WilsonCoefficient("C_S1_bc_THDM", GroupMapper::str(WGroup::BCC_bc, ScaleType::MATCHING))
{
...
double C_S1_bc_THDM::compute_LO(const std::unordered_map<ParamId, std::shared_ptr<Parameter>>& src) {
...
C_S2_bc_THDM::C_S2_bc_THDM()
    : WilsonCoefficient("C_S2_bc_THDM", GroupMapper::str(WGroup::BCC_bc, ScaleType::MATCHING))
{
    matching_info[QCDOrder::LO] = {
        {
            {"WPARAM_SI_BSM", 7},              // lu
...
double C_S2_bc_THDM::compute_LO(const std::unordered_map<ParamId, std::shared_ptr<Parameter>>& src) {
...
C_S1_bu_THDM::C_S1_bu_THDM()
    : WilsonCoefficient("C_S1_bu_THDM", GroupMapper::str(WGroup::BCC_bu, ScaleType::MATCHING))
{
    matching_info[QCDOrder::LO] = {
        {
            {"WPARAM_SI_BSM", 8},              // ld
...
double C_S1_bu_THDM::compute_LO(const std::unordered_map<ParamId, std::shared_ptr<Parameter>>& src) {
    double ld     = src.at({ParameterType::WILSON, "WPARAM_SI_BSM", 8})->get_val();
    double mH     = src.at({ParameterType::BSM, "MASS", 37})->get_val();
    double m_tau  = src.at({ParameterType::SM, "MASS", 15})->get_val();
    double l_tau  = src.at({ParameterType::BSM, "YE", {3, 3}})->get_val();
...
C_S2_bu_THDM::C_S2_bu_THDM()
    : WilsonCoefficient("C_S2_bu_THDM", GroupMapper::str(WGroup::BCC_bu, ScaleType::MATCHING))
{
    matching_info[QCDOrder::LO] = {
        {
            {"WPARAM_SI_BSM", 7},              // lu
...
double C_S2_bu_THDM::compute_LO(const std::unordered_map<ParamId, std::shared_ptr<Parameter>>& src) {
    double lu     = src.at({ParameterType::WILSON, "WPARAM_SI_BSM", 7})->get_val();
    double mH     = src.at({ParameterType::BSM, "MASS", 37})->get_val();
    double m_c    = src.at({ParameterType::SM, "MASS", 4})->get_val();    // m_c
    double m_tau  = src.at({ParameterType::SM, "MASS", 15})->get_val();   // m_tau
