...
C_S1_du_THDM::C_S1_du_THDM()
    : WilsonCoefficient("C_S1_du_THDM", GroupMapper::str(WGroup::CC_du, ScaleType::MATCHING))
{
...
double C_S1_du_THDM::compute_LO(const std::unordered_map<ParamId, std::shared_ptr<Parameter>>& src) {
...
C_S2_du_THDM::C_S2_du_THDM()
    : WilsonCoefficient("C_S2_du_THDM", GroupMapper::str(WGroup::CC_du, ScaleType::MATCHING))
{
    matching_info[QCDOrder::LO] = {
        {
            {"WPARAM_SI_BSM", 7},              // lu
...
double C_S2_du_THDM::compute_LO(const std::unordered_map<ParamId, std::shared_ptr<Parameter>>& src) {
