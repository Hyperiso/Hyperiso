...
void CoefficientManager::ensure_sm_intermediate_and_copy_to_final(
    ...
    // HYP_AS_SM_MARTY means: keep MARTY for the BSM piece, but take the
    // published Hyperiso/SuperIso SM contribution from the built-in backend at
    // the requested QCD order.  This must apply at LO as well as at NLO/NNLO:
    // otherwise a LO export still receives MARTY's SM_Model value instead of
    // Hyperiso's hard-coded SM value.
    bool allow_hardcoded_sm = (marty_backend && hard_lo);
    if (allow_hardcoded_sm && !ports_config.build_group) {
        LOG_WARN(
            "(CoefficientManager) HYP_AS_SM_MARTY=true but no ports_config.build_group provided; "
            "cannot build a built-in SM group. Falling back to MARTY SM at LO."
        );
        allow_hardcoded_sm = false;
    }

    const bool sm_use_marty = allow_hardcoded_sm ? false : marty_backend;
    // The SM intermediate is always a genuine SM-model contribution.  When
    // HYP_AS_SM_MARTY is active, it is built by Hyperiso's built-in SM backend;
    // otherwise it is generated by MARTY's SM_Model in pure-MARTY mode.
    // BSM-specific TOTAL-SM-like subtractions remain inside the generated BSM
    // MARTY library.
