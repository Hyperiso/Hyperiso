...
void WilsonParameterHelper::init_scale_independent_block(int gen) {
    ...
    auto func = [gen] (const std::unordered_map<std::string, std::shared_ptr<Block>>& src, std::shared_ptr<DependentBlock> dep_block) {
		...
		int nf = 5;
		int id = 1;
        dep_block->store_or_assign(id, Parameter({ParamId{ParameterType::WILSON, "WPARAM_SI_SM", id++}}, xh, 0., 0.));
		dep_block->store_or_assign(id, Parameter({ParamId{ParameterType::WILSON, "WPARAM_SI_SM", id++}}, gen, 0., 0.));
		dep_block->store_or_assign(id, Parameter({ParamId{ParameterType::WILSON, "WPARAM_SI_SM", id++}}, src.at("MASS")->retrieve(9 + 2 * gen).get_val(), 0., 0.));
		dep_block->store_or_assign(id, Parameter({ParamId{ParameterType::WILSON, "WPARAM_SI_SM", id++}}, 0.22305, 0., 0.)); //TODO, sw2
		dep_block->store_or_assign(id, Parameter({ParamId{ParameterType::WILSON, "WPARAM_SI_SM", id++}}, 11.-2./3.*nf, 0., 0.)); //TODO, beta0
    };

    WilsonParameterHelper::composer.compose("WPARAM_SI_SM", src, func);
}
...
void WilsonParameterHelper::init_matching_block(double mu_W) {
    ...
    auto func = [mu_W] (const std::unordered_map<std::string, std::shared_ptr<Block>>& src, std::shared_ptr<DependentBlock> dep_block) {
		...
		dep_block->store_or_assign(1, Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", 1}}, alphas_muW, 0., 0.));
		dep_block->store_or_assign(LhaID(2, 1), Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", LhaID(2, 1)}}, xt, 0., 0.));
		dep_block->store_or_assign(LhaID(2, 2), Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", LhaID(2, 2)}}, std::pow(xt, 2), 0., 0.));
		dep_block->store_or_assign(LhaID(2, 3), Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", LhaID(2, 3)}}, std::pow(xt, 3), 0., 0.));
		dep_block->store_or_assign(LhaID(2, 4), Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", LhaID(2, 4)}}, std::pow(xt, 4), 0., 0.));
		dep_block->store_or_assign(3, Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", 3}}, L, 0., 0.));
		dep_block->store_or_assign(4, Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", 4}}, mass_c_muW, 0., 0.));
		dep_block->store_or_assign(LhaID(5, 1), Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", LhaID(5, 1)}}, mass_b_muW_mbrun, 0., 0.));
		dep_block->store_or_assign(LhaID(5, 2), Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", LhaID(5, 2)}}, mass_b_muW_mbpole, 0., 0.));
		dep_block->store_or_assign(6, Parameter({ParamId{ParameterType::WILSON, "WPARAM_MATCH_SM", 6}}, mass_top_muW, 0., 0.));
    };

    WilsonParameterHelper::composer.compose("WPARAM_MATCH_SM", src, func);
}
...
void WilsonParameterHelper::init_running_block(double mu_W, double mu_h) {
    ...
    auto func = [mu_W, mu_h] (const std::unordered_map<std::string, std::shared_ptr<Block>>& src, std::shared_ptr<DependentBlock> dep_block) {
		...
		dep_block->store_or_assign(1, Parameter({ParamId{ParameterType::WILSON, "WPARAM_RUN_SM", 1}}, alphas_mu, 0., 0.));
		dep_block->store_or_assign(2, Parameter({ParamId{ParameterType::WILSON, "WPARAM_RUN_SM", 2}}, eta, 0., 0.));
    };

    WilsonParameterHelper::composer.compose("WPARAM_RUN_SM", src, func);
}
