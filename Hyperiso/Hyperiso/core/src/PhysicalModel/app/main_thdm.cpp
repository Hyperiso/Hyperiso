...
int main() {
    ...
    config_hyp.mty_model_name = "THDM";
    config_hyp.mty_model_path = std::string(project_tp_root.data()) + "MARTY/src/MARTY/src/marty/models/thdm.h";
    ...
    WilsonBuildConfig config({GroupMapper::to_id(WGroup::BScalar), GroupMapper::to_id(WGroup::B), GroupMapper::to_id(WGroup::BPrime)}, 81, 42, QCDOrder::LO);
    std::cout << "here " << std::endl;
    wi.build(config);
    std::cout << "here " << std::endl;
