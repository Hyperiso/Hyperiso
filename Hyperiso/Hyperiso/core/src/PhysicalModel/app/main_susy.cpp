...
int main() {
    ...
    hyp.init("/home/cern/hyperiso/Hyperiso/Hyperiso/core/Test/InputFiles/testInput.slha", config_hyp); // Initialize program manager with LHA file
    ...
    for (auto elem : {WCoef::CQ1, WCoef::CQ2}) {
        LOG_INFO(WCoefMapper::str(elem), "(MW) at LO =", wi.getR(WGroup::BScalar, elem, QCDOrder::LO, ContributionType::BSM));
        LOG_INFO(WCoefMapper::str(elem), "(MW) at NLO =", wi.getR(WGroup::BScalar, elem, QCDOrder::NLO, ContributionType::BSM));
        LOG_INFO("\n");
    }

    for (auto elem : {WCoef::C1, WCoef::C2, WCoef::C3, WCoef::C4, WCoef::C5, WCoef::C6, WCoef::C7, WCoef::C8, WCoef::C9, WCoef::C10}) {
        LOG_INFO(WCoefMapper::str(elem), "(MW) at LO =", wi.getR(WGroup::B, elem, QCDOrder::LO, ContributionType::BSM));
        LOG_INFO(WCoefMapper::str(elem), "(MW) at NLO =", wi.getR(WGroup::B, elem, QCDOrder::NLO, ContributionType::BSM));
        LOG_INFO(WCoefMapper::str(elem), "(MW) at NNLO =", wi.getR(WGroup::B, elem, QCDOrder::NNLO, ContributionType::BSM));
        LOG_INFO("\n");
    }
    ...
    for (auto elem : {WCoef::CP1, WCoef::CP2, WCoef::CP3, WCoef::CP4, WCoef::CP5, WCoef::CP6, WCoef::CP7, WCoef::CP8, WCoef::CP9, WCoef::CP10, WCoef::CPQ1, WCoef::CPQ2}) {
        LOG_INFO(WCoefMapper::str(elem), "(MW) at LO =", wi.getR(WGroup::BPrime, elem, QCDOrder::LO, ContributionType::BSM));
        LOG_INFO(WCoefMapper::str(elem), "(MW) at NLO =", wi.getR(WGroup::BPrime, elem, QCDOrder::NLO, ContributionType::BSM));
        LOG_INFO("\n");
    }
