...
RealMatrix cholesky_L(RealMatrix R) {
    ...
    gsl_matrix_ptr R_gsl = R.to_gsl_matrix();
...
gsl_matrix_ptr RealMatrix::to_gsl_matrix() const {
    gsl_matrix_ptr A = make_gsl_matrix(rows_, cols_);
...
EigenSystem RealMatrix::eig() const {
    ...
    gsl_matrix_ptr M = this->to_gsl_matrix();
    gsl_vector_ptr eval = make_gsl_vector (rows_);
    gsl_matrix_ptr evec = make_gsl_matrix (rows_, rows_);
    gsl_eigen_workspace_ptr w = make_eigen_workspace(rows_);
...
SignedLogDet RealMatrix::slogdet() const {
    ...
    gsl_matrix_ptr M = this->to_gsl_matrix();
    gsl_permutation_ptr p = make_gsl_permutation(rows_);
    int p_sign;
...
RealMatrix RealMatrix::inv() const {
    ...
    gsl_matrix_ptr M = this->to_gsl_matrix();
    gsl_matrix_ptr M_inv = make_gsl_matrix(rows_, cols_);
    gsl_permutation_ptr p = make_gsl_permutation(rows_);
    int p_sign;
