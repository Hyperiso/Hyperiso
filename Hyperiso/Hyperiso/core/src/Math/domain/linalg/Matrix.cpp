...
RealMatrix RealMatrix::inv() const {
    ...
    const int decomp_status = gsl_linalg_LU_decomp(M.get(), p.get(), &p_sign);
    if (decomp_status != GSL_SUCCESS) {
        gsl_set_error_handler(old_handler);
        throw std::runtime_error("Matrix LU decomposition failed");
    }

    const int invert_status = gsl_linalg_LU_invert(M.get(), p.get(), M_inv.get());
    gsl_set_error_handler(old_handler);

    if (invert_status != GSL_SUCCESS) {
        throw std::runtime_error("Matrix is singular");
    }
