...
struct ParametersAccessRights {
    static inline const std::map<std::string, std::unordered_set<LhaID>> SM_RIGHTS {
        {"MASS", {1, 2, 3, 4, 11, 12, 13, 14, 15, 16, 21, 22, 24, 25}}, 
        {"GAUGE", {1, 2, 3}},
    };

    static inline const std::map<std::string, std::unordered_set<LhaID>> THDM_RIGHTS {
        {"MASS", {25, 35, 36, 37}}, 
        {"GAUGE", {{}}},
    };

    static inline const std::map<std::string, std::unordered_set<LhaID>> SUSY_RIGHTS {
        {"MASS", {25, 35, 36, 37, 
