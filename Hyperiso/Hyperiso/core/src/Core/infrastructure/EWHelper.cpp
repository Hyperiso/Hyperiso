...
double EWHelper::pi_light_heavy(double mu, double m_q_pole, double n_l, double alpha) {
    double l_mu = std::log(std::pow(mu / m_q_pole, 2));
    double a_s = QCDHelper::alpha_s(mu) / PI;
    double Q_factor = Q_factor = 2 * (4 / 9) + (n_l - 2) * (1 / 9);
...
double EWHelper::alpha_4_mu_b(double inv_alpha_m_Z, double m_Z, double m_b_pole, double mu_b) {
    ...
    double delta_b = 4 / 3 * pi_heavy(mu_b, -2 / 3, m_b_pole, 4, 1 / inv_alpha_m_Z) + pi_light_heavy(mu_b, m_b_pole, 4, 1 / inv_alpha_m_Z);
