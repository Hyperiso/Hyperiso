...
int main() {
    ...
    for (auto [g_id, _] : GroupMapper::mapping()) {
        for (auto c_id : WCoefMapper::get_group(g_id)) {
            LOG_INFO(WCoefMapper::str(c_id), "=", wi.getM(g_id, c_id, QCDOrder::NLO, ContributionType::SM));
        }
    }
    ...
    for (auto [g_id, _] : GroupMapper::mapping()) {
        for (auto c_id : WCoefMapper::get_group(g_id)) {
            LOG_INFO(WCoefMapper::str(c_id), "=", wi.getR(g_id, c_id, QCDOrder::NLO, ContributionType::SM));
        }
    }
