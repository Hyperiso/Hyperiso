...
struct BDlnuConfig : public DecayConfig {
 ...
 */
class BDlnuDecay : public DecayParentConfigurable<BDlnuConfig> {
private:
...
public:
    BDlnuDecay(QCDOrder order, double matching_scale, double hadronic_scale, ObservablePortsConfig& ports) : DecayParentConfigurable(DecayMapper::to_id(Decays::B__D_l_nu), matching_scale, hadronic_scale, order, ports) {
    ...
    void set_config_spe(BDlnuConfig config) override {this->cfg = config;}
};
