...
scalar_t BllDecay::W1(scalar_t r, bool prime) {
    scalar_t cq1 = w_proxy->getFR(WGroup::BScalar, WCoef::CQ1, QCDOrder::NLO);
    scalar_t cpq1 = prime ? w_proxy->getFR(WGroup::BPrime, WCoef::CPQ1, QCDOrder::NLO) : 0;
    return r * (cq1 - cpq1);
}

scalar_t BllDecay::W2Q(scalar_t r, bool prime) {
    scalar_t cq2 = w_proxy->getFR(WGroup::BScalar, WCoef::CQ2, QCDOrder::NLO);
    scalar_t cpq2 = prime ? w_proxy->getFR(WGroup::BPrime, WCoef::CPQ2, QCDOrder::NLO) : 0;
    return r * (cq2 - cpq2);
}

scalar_t BllDecay::W210(scalar_t x, bool prime) {
    scalar_t c10 = w_proxy->getFR(WGroup::B, WCoef::C10, QCDOrder::NNLO);
    scalar_t cp10 = prime ? w_proxy->getFR(WGroup::BPrime, WCoef::CP10, QCDOrder::NLO) : 0;
    return 2. * (c10 - cp10) * x;
}
...
scalar_t BllDecay::A_DG(scalar_t x, scalar_t r) {
    scalar_t cq1 = w_proxy->getFR(WGroup::BScalar, WCoef::CQ1, QCDOrder::NLO);
    scalar_t cpq1 = w_proxy->getFR(WGroup::BPrime, WCoef::CPQ1, QCDOrder::LO);
    scalar_t cq2 = w_proxy->getFR(WGroup::BScalar, WCoef::CQ2, QCDOrder::NLO);
    scalar_t cpq2 = w_proxy->getFR(WGroup::BPrime, WCoef::CPQ2, QCDOrder::LO);
    scalar_t c10 = w_proxy->getFR(WGroup::B, WCoef::C10, QCDOrder::NNLO);
    scalar_t cp10 = w_proxy->getFR(WGroup::BPrime, WCoef::CP10, QCDOrder::LO);
    scalar_t c10sm = w_proxy->getFR(WGroup::B, WCoef::C10, QCDOrder::NNLO, true);

    scalar_t S = x * std::sqrt(1. - 4. * x * x) * r * (cq1 - cpq1) / (2. * c10sm);
    scalar_t P = (c10 - cp10) / c10sm + x * r * (cq2 - cpq2) / (2. * c10sm);
