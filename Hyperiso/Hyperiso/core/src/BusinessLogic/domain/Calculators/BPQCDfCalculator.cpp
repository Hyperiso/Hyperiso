...
complex_t BPQCDfCalculator::T_P(double q2, bool bar) {
    ...
    if (this->ff_tp == B_FF_Type::SOFT) {
        C_P = -(C_par_0(q2, 1, bar) + this->alpha_s_mu_b * ObsQCDProxy().get_constants()->C_F / (4. * PI) * (C_par_f(q2, 1, bar) + C_par_nf(q2, bar)));
    } else {
        C_P = -this->alpha_s_mu_b * ObsQCDProxy().get_constants()->C_F / (4. * PI) * C_par_nf(q2, bar);
    }
...
double BPQCDfCalculator::Delta_P_0(double q2) {
    return 1. + this->alpha_s_mu_f * ObsQCDProxy().get_constants()->C_F / (2. * PI) * (
        1. - L(q2) + 3. * PI2 * (m_B - 2 * E(q2)) * this->f_B * this->f_X_par / (ObsQCDProxy().get_constants()->Nc * this->lambda_B_p * this->ff_calculator->get(BP_FF::XI_P, q2) * std::pow(this->E(q2), 2)) * (1 + this->a_1_par + this->a_2_par)
    );
}

double BPQCDfCalculator::Delta_P_T(double q2) {
    return 1. + this->alpha_s_mu_b * ObsQCDProxy().get_constants()->C_F / (2. * PI) * (
        L(q2) - this->L_b + 6. * PI2 * this->f_B * this->f_X_par / (ObsQCDProxy().get_constants()->Nc * this->lambda_B_p * this->ff_calculator->get(BP_FF::XI_P, q2) * this->E(q2)) * (1 + this->a_1_par + this->a_2_par)
    );
}
