...
PlnuCalculator::PlnuCalculator(int P_id, int l_id, complex_t C_A, complex_t C_P) {
    ...
    this->tau_P = p(ParamId{ParameterType::FLAVOR, "FLIFE", P_id}) / HBAR;
    ...
    this->m_qd = allowed_P.at(P_id)[1] == 5 ? p(ParamId{ParameterType::SM, "SMINPUTS", 5}) : p(ParamId{ParameterType::SM, "MASS", allowed_P.at(P_id)[1]});
