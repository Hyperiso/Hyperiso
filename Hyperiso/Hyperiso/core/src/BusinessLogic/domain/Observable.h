...
public:
    Observable(Observables id, std::shared_ptr<DecayParent> decay_parent, scalar_t exp_val = 0.0) : id(id), decay_parent(decay_parent), exp_val(exp_val) {}

    Observables getId() const { return id; }
    scalar_t get_exp_val() const { return exp_val; }
    void set_exp_val(scalar_t value) { exp_val = value; }
