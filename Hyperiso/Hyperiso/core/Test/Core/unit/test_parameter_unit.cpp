...
static void test_assignment_and_operators() {
    ...
    sp1 += sp3; 
    assert(sp1->get_id() == sp3->get_id());
    assert(std::abs(sp1->get_val() - 5.0) < 1e-12);

    sp1 * 2.0; 
    assert(std::abs(sp1->get_val() - 10.0) < 1e-12);
}

static void test_dependent_parameter_lazy_dirty_basic() {
    std::cout << "\n-- [Unit] DependentParameter lazy + dirty --\n";

    auto p1 = std::make_shared<Parameter>(ParamId{ParameterType::SM, "SRC", 1}, 0.5, 0.0, 0.0);
    auto p2 = std::make_shared<Parameter>(ParamId{ParameterType::SM, "SRC", 2}, 1.0, 0.0, 0.0);
    ...
    auto dep = std::make_shared<DependentParameter>(
        ParamId{ParameterType::SM, "DEP", 100},
        sources,
        lambda
    );
    dep->init();
...
static void test_dependent_freeze_unfreeze_clear_above() {
    std::cout << "\n-- [Unit] DependentParameter freeze/unfreeze/clear_above (lazy) --\n";
    ...
    auto lambda = [&](const auto& srcs, std::shared_ptr<DependentParameter> self) {
        ...
        self->set_expected(10.0 * x);
    };

    auto dep = std::make_shared<DependentParameter>(
        ParamId{ParameterType::SM, "DEP", 200},
        srcs,
        lambda
    );
    dep->init();
