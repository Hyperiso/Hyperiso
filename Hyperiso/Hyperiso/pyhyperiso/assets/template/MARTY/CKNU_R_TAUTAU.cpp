...
int calculate_CKNU_R_TAUTAU(Model &model, gauge::Type gauge) {
    ...
    const std::vector<Insertion> hyperiso_knunu_insertions = {
        ...
        Outgoing("nu_tau"), Outgoing(AntiPart("nu_tau"))
    };
    ...
    if (hyperiso_marty_order == mty::Order::TreeLevel) {
        C = hyperiso_marty_project_knunu_tree_recipe(
            ...
            mty::DiracCoupling::VR, mty::DiracCoupling::VL,
            {1, 2, 0, 3}
        );
    } else {
        auto O = dimension6Operator(
            model, wil, mty::DiracCoupling::VR, mty::DiracCoupling::VL,
            {1, 2, 0, 3});
        C = getWilsonCoefficient(wil, O);
    }

    [[maybe_unused]] int sysres = system("rm -rf libs/CKNU_R_TAUTAU_SM");
    mty::Library wilsonLib("CKNU_R_TAUTAU_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("CKNU_R_TAUTAU", C);
...
int main() {
    SM_Model sm;
    return calculate_CKNU_R_TAUTAU(sm, gauge::Type::Feynman);
}
