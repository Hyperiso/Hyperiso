...
int calculate_CNU_R_EMU(Model &model, gauge::Type gauge) {
    ...
    const std::vector<Insertion> hyperiso_bnunu_insertions = {
        ...
        Outgoing("nu_e"), Outgoing(AntiPart("nu_mu"))
    };
    ...
    [[maybe_unused]] int sysres = system("rm -rf libs/CNU_R_EMU_SM");
    mty::Library wilsonLib("CNU_R_EMU_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("CNU_R_EMU", C);
...
int main() {
    SM_Model sm;
    return calculate_CNU_R_EMU(sm, gauge::Type::Feynman);
}
