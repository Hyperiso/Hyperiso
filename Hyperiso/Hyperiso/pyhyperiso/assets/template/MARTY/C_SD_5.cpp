...
int calculate_C_SD_5(Model &model, gauge::Type gauge) {
    ...
    auto wil_t = model.computeWilsonCoefficients(mty::Order::OneLoop, 
        {Incoming("s"), Incoming(AntiPart("d")), Outgoing(AntiPart("s")), Outgoing("d")}, 
        opts);

    auto O = dimension6Operator(model, wil_t, mty::DiracCoupling::L, mty::DiracCoupling::R, {"C", mty::ColorCoupling::Crossed});
    Expr C = getWilsonCoefficient(wil_t, O);

    [[maybe_unused]] int sysres = system("rm -rf libs/C_SD_5_SM");
    mty::Library wilsonLib("C_SD_5_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("C_SD_5", C);
...
int main() {
    SM_Model sm;
    return calculate_C_SD_5(sm, gauge::Type::Feynman);
}