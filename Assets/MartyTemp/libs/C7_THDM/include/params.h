...
struct param_t {
    ...
    void reset()
    {
        using real_params = std::array<csl::InitSanitizer<real_t>*, 15>;
        using complex_params = std::array<csl::InitSanitizer<complex_t>*, 3>;

        for (auto &par : real_params{
                &G_F, &M_W, &m_b, &m_c, &m_s, 
                &m_t, &m_u, &V_cb, &V_tb, &V_us, &beta, 
                &e_em, &m_Hp, &s_12, &theta_W, })
        {
            par->reset();
        }
    ...
    {
        using real_params = std::array<csl::InitSanitizer<real_t> const*, 15>;
