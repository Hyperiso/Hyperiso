...
int calculate_C_SD_4(Model &model, gauge::Type gauge) {
    ...
    auto O = dimension6Operator(model, wil_t, mty::DiracCoupling::L, mty::DiracCoupling::R);
    Expr C = getWilsonCoefficient(wil_t, O);

    [[maybe_unused]] int sysres = system("rm -rf libs/C_SD_4_SM");
    mty::Library wilsonLib("C_SD_4_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("C_SD_4", C);
...
int main() {
    SM_Model sm;
    return calculate_C_SD_4(sm, gauge::Type::Feynman);
}