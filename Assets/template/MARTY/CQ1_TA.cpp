...
int calculate_CQ1tau(Model &model, gauge::Type gauge) {
    ...
    Expr CQ1_tau = getWilsonCoefficient(wil, Q1_mu);

    [[maybe_unused]] int sysres = system("rm -rf libs/CQ1_TA_SM");
    mty::Library wilsonLib("CQ1_TA_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("CQ1_TA", CQ1_tau);
...
int main() {
    SM_Model sm;
    return calculate_CQ1tau(sm, gauge::Type::Feynman);
}