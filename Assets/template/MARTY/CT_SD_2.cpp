...
int calculate_CT_SD_2(Model &model, gauge::Type gauge) {
    ...
    auto O = dimension6Operator(model, wil_t, mty::DiracCoupling::R, mty::DiracCoupling::R);
    Expr C = getWilsonCoefficient(wil_t, O);

    [[maybe_unused]] int sysres = system("rm -rf libs/CT_SD_2_SM");
    mty::Library wilsonLib("CT_SD_2_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("CT_SD_2", C);
...
int main() {
    SM_Model sm;
    return calculate_CT_SD_2(sm, gauge::Type::Feynman);
}