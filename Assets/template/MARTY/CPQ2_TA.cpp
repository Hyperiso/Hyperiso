...
int calculate_CQP2tau(Model &model, gauge::Type gauge) {
    ...
    Expr CQP2_tau = getWilsonCoefficient(wil, QP2_mu);

    [[maybe_unused]] int sysres = system("rm -rf libs/CPQ2_TA_SM");
    mty::Library wilsonLib("CPQ2_TA_SM", "libs");
    wilsonLib.cleanExistingSources();
    wilsonLib.addFunction("CPQ2_TA", CQP2_tau);
...
int main() {
    SM_Model sm;
    return calculate_CQP2tau(sm, gauge::Type::Feynman);
}